...
| 2.4 | UI badges read `source` | P0 | S | Done | Stop deriving provenance by string-matching storage paths; the API returns `source`/`verified` and the gallery renders from that. |
...
| 2.8 | SVG renderer v2 | P1 | L | Done | Keep the exact-count guarantee, lose the spreadsheet look: 3D cube faces with highlight/shadow, show-style eyes and mouth, proper limbs, Numberling above the head, correct stacking for tens/hundreds. |
